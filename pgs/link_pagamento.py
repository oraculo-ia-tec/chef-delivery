...
async def showLinks():
    ...
    with st.form(key='create_link_pagamento'):
        ...
        if submit_button:
            ...
            import requests
            response = requests.post(f"{BASE_URL}/paymentLinks", headers={'access_token': ''}, json=novo_link.dict())
    ...
    if st.button("Carregar Links de Pagamento"):
        with st.spinner("Carregando lista de links de pagamento..."):
            import requests
            response = requests.get(f"{BASE_URL}/paymentLinks", headers={'access_token': ''})
