...
async def _build_client():
    ...
    if not api_key:
        raise ValueError(
            "Informe uma chave de API do Asaas para executar os testes.")
...
async def showTesteApi() -> None:
    ...
    status_col.metric("Chave ativa", _mask_secret(
        st.session_state.asaas_test_api_key))
    timeout_col.metric(
        "Timeout", f"{float(st.session_state.asaas_test_timeout):.0f}s")
    ...
    with config_tab:
        with st.form("asaas-config-form"):
            ...
            submitted = st.form_submit_button(
                "Salvar configuracao e testar conexao")
        ...
        st.info(
            "A aba de configuracao salva o ambiente ativo e executa um healthcheck local do cliente Asaas.")
    ...
    with customers_tab:
        ...
        with create_col:
            with st.form("asaas-create-customer-form"):
                ...
                customer_name = st.text_input(
                    "Nome", value="Cliente Teste Chef Delivery")
                customer_email = st.text_input(
                    "Email", value="cliente.teste@chefdelivery.com")
                customer_cpf = st.text_input(
                    "CPF ou CNPJ", value="12345678909")
                customer_phone = st.text_input("Celular", value="31999999999")
                customer_reference = st.text_input(
                    "Referencia externa", value="teste-cliente-streamlit")
                customer_notes = st.text_area(
                    "Observacoes", value="Cliente criado pela pagina de teste da API.")
                create_customer_submitted = st.form_submit_button(
                    "Enviar criacao de cliente")
            ...
            if create_customer_submitted:
                ...
                try:
                    ...
                    _store_success("create_customer",
        ...
        with query_col:
            with st.form("asaas-customer-query-form"):
                ...
                list_external_reference = st.text_input(
                    "Filtro por referencia externa")
                list_limit = st.number_input(
                    "Limite", min_value=1, max_value=100, value=10, step=1)
                query_mode = st.radio("Operacao", options=[
                                      "Consultar por ID", "Listar clientes"], horizontal=True)
                customer_query_submitted = st.form_submit_button(
                    "Executar consulta de clientes")
                    ...
                    async with client:
                        if query_mode == "Consultar por ID":
                            if not customer_id.strip():
                                raise ValueError(
                                    "Informe o ID do cliente para consulta individual.")
    ...
    with payments_tab:
        ...
        with create_payment_col:
            with st.form("asaas-create-payment-form"):
                ...
                payment_customer_id = st.text_input(
                    "ID do cliente", help="Use um customer id retornado pelo Asaas.")
                payment_billing_type = st.selectbox("Tipo de cobranca", options=[
                                                    "PIX", "BOLETO", "CREDIT_CARD", "UNDEFINED"])
                payment_value = st.number_input(
                    "Valor", min_value=0.01, value=25.0, step=1.0)
                payment_due_date = st.date_input(
                    "Data de vencimento", value=date.today() + timedelta(days=1), format="DD/MM/YYYY")
                payment_description = st.text_input(
                    "Descricao", value="Teste manual de cobranca via Streamlit")
                payment_reference = st.text_input(
                    "Referencia externa", value="teste-pagamento-streamlit")
                create_payment_submitted = st.form_submit_button(
                    "Criar pagamento")
            ...
            if create_payment_submitted:
                ...
                try:
                    if not payment_customer_id.strip():
                        raise ValueError(
                            "Informe o ID do cliente para criar o pagamento.")
                    client = await _build_client()
                    async with client:
                        response_data = await client.create_payment(payload)
                    _store_success("create_payment",
                                   request_data, response_data)
                except Exception as exc:
        ...
        with get_payment_col:
            with st.form("asaas-payment-query-form"):
                ...
                payment_reference_filter = st.text_input(
                    "Filtro por referencia externa")
                payment_limit = st.number_input(
                    "Limite de pagamentos", min_value=1, max_value=100, value=10, step=1)
                payment_mode = st.radio("Operacao de pagamento", options=[
                                        "Consultar por ID", "Listar pagamentos"], horizontal=True)
                payment_query_submitted = st.form_submit_button(
                    "Executar consulta de pagamentos")
                    ...
                    async with client:
                        if payment_mode == "Consultar por ID":
                            if not payment_id.strip():
                                raise ValueError(
                                    "Informe o ID do pagamento para consulta individual.")
    ...
    with pix_tab:
        with st.form("asaas-pix-qrcode-form"):
            ...
            pix_qrcode_submitted = st.form_submit_button(
        ...
        if pix_qrcode_submitted:
            ...
            try:
                if not pix_payment_id.strip():
                    raise ValueError(
                        "Informe o ID do pagamento para consultar o QR Code PIX.")
                client = await _build_client()
                async with client:
                ...
                if qr_code.encoded_image:
                    st.image(
                        f"data:image/png;base64,{qr_code.encoded_image}", caption="QR Code PIX retornado pelo Asaas")
            except Exception as exc:
