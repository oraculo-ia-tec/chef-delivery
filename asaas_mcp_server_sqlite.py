...
from asaas_repository_sqlite import SQLitePaymentRepository

ASAAS_API_KEY = os.getenv("ASAAS_API_KEY", "")
ASAAS_ENVIRONMENT = os.getenv("ASAAS_ENVIRONMENT", "sandbox")
ASAAS_SQLITE_PATH = os.getenv(
    "ASAAS_SQLITE_PATH", "output/chef_delivery_asaas.db")

_repository = SQLitePaymentRepository(ASAAS_SQLITE_PATH)
...
async def mcp_list_persisted_payments(payload: dict[str, Any]) -> dict[str, Any]:
    limit = int(payload.get("limit", 20))
    return {"payments": await _repository.list_payments(limit=limit)}


async def mcp_list_persisted_webhooks(payload: dict[str, Any]) -> dict[str, Any]:
    limit = int(payload.get("limit", 20))
    return {"webhooks": await _repository.list_webhooks(limit=limit)}
...
TOOLS = {
    ...
    "list_persisted_payments": {
        "description": "Lista pagamentos persistidos no SQLite do Chef Delivery.",
        "handler": mcp_list_persisted_payments,
    },
    "list_persisted_webhooks": {
        "description": "Lista webhooks persistidos no SQLite do Chef Delivery.",
        "handler": mcp_list_persisted_webhooks,
    },
}
