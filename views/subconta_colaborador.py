...
async def criar_subcontaColaborador(subconta: Subaccount):
...
async def showSbcontaColaborador():
    st.title("Sistema Flash Pagamentos")

    st.header("Novo Colaborador")
    ...
    with st.form(key='form_subconta'):
        ...
        if submit_button:
            ...
            st.success(f"Colaborador cadastrado com sucesso! Nome: {nome}")
            ...
            try:
                ...
                st.error(f"Erro ao criar novo colaborador: {e.detail}")

if __name__ == "__main__":
    asyncio.run(showSbcontaColaborador())
